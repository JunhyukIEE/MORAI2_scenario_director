    ...
    for (double inside_off : inside_offsets) {
      ...
      for (size_t i = 0; i < indices.size(); ++i) {
        ...
        if (i >= brake_idx && i <= apex_idx) {
          ...
          // 탈출 구간: 가속 (execute_speed_boost 사용)
...
PlanResult LocalPathPlanner::plan(
  ...
  if (check_gap) {
    ...
    double ref_y = ref_->y()[opp_idx];
    double ref_yaw = ref_->yaw()[opp_idx];
    double dx_to_opp = opp_x - ref_x;
  ...
  for (double lat : config_.lateral_offsets) {
    ...
    for (double sc : config_.speed_scales) {
      ...
      debug_cand.speed_scale = sc;
      debug_cand.is_late_braking = false;
      for (size_t i = 0; i < cand_x.size(); ++i) {
        debug_cand.path.emplace_back(cand_x[i], cand_y[i]);
      }
  ...
  if (overtake_active && has_overtake_apex) {
    ...
    for (const auto& lb_cand : lb_candidates) {
      ...
      // 브레이킹 구간 중간 지점의 실제 lateral offset 계산
      double avg_lat_offset = 0.0;
      if (!lb_cand.path_x.empty() && !idxs.empty()) {
        // 브레이킹 포인트와 apex 사이의 중간 지점 사용 (가장 바깥쪽 지점)
        ...
        // 정확한 lateral offset: reference line의 법선 방향으로 투영
        double ref_yaw = ref_->yaw()[ref_idx];
        double nx = -std::sin(ref_yaw);  // 법선 벡터 (오른쪽이 양수)
        double ny = std::cos(ref_yaw);
        avg_lat_offset = dx * nx + dy * ny;  // 양수 = reference line 오른쪽
      }
      ...
      PlanResult::DebugCandidate debug_cand;
      debug_cand.reward = R;
      debug_cand.lat_offset = avg_lat_offset;
      debug_cand.speed_scale = 1.0;
      debug_cand.is_late_braking = true;
      for (size_t i = 0; i < lb_cand.path_x.size(); ++i) {
        debug_cand.path.emplace_back(lb_cand.path_x[i], lb_cand.path_y[i]);
      }
      debug_results.push_back(debug_cand);
  ...
  // 모든 후보 저장 (시각화용)
  best.all_candidates = debug_results;
